...
| `lib/whatsappModeles.js` | 📲 Les modèles de messages WhatsApp approuvés par Meta et l'ordre de leurs trous, y compris le choix entre un rappel de dette ordinaire et un rappel d'échéance — **le serveur lit CE fichier**, l'alerte à l'administrateur quand un client demande un conseiller (`alerte_conseiller`, envoyée par le serveur seul), et aucun mot de passe n'y entre |
...
| `lib/assistantWhatsapp.js` | 🤖 L'assistant du numéro WhatsApp BMI (24/09/2026) : le mot d'accueil et les huit lignes du menu écrits par Timo, quand le robot répond et quand il se TAIT (conversation confiée, employé qui a répondu, conseiller demandé), ce qu'il dit avec NOS données (prix et « disponible / sur commande », jamais la quantité, jamais une dette), la demande de devis qui devient une fiche 🧲 Prospects, la ligne qu'il écrit, le client qui ATTEND un conseiller (`attenteConseiller`), la description d'article dite aux clients (`note_assistant`, jamais les notes internes) — **le serveur lit CE fichier** (`api/whatsapp-entrant.js`), sans IA, rien ne sort du Togo |
