...
// Les supports d'une boutique : tout article dont le nom ou la catégorie
// porte le mot « support » (jamais un étrier). C'est LA liste proposée dans
...
export function DimensionnementSolaire({ db, profile, save, onConvertirEnVente, devisAReprendre, onDevisRepriseConsomme, bq, setBq }) {
  ...
  // Le catalogue (lib/appareils.js) est PROPOSÉ sous le champ ; la
  // puissance ne se pré-remplit que quand la personne CLIQUE une
  // proposition (onChoisir). Ce qu'elle tape n'est jamais transformé —
  // taper « tv » ou effacer lettre à lettre ne remplit rien (Timo,
  // 09/09/2026 : « ça ne me donne pas la liberté de choisir moi-même »).
  const catalogue = catalogueAppareils(db, profile);
  const propositionsAppareils = suggestionsAppareils(catalogue);
  const choisirAppareil = (id, proposition) => {
    const e = catalogue.find((x) => x.id === proposition.cle) || appareilDuCatalogue(catalogue, proposition.valeur);
    if (!e) return;
    setAppareils((liste) => liste.map((a) => (a.id === id ? { ...a, nom: e.nom, puissance: String(e.puissance) } : a)));
  };
  ...
  useEffect(() => {
    ...
    setSupportId(supportDepuisLignes(lignesReprises, produitsBoutique)?.id || null);
  ...
  // dans le devis, dans UNE liste déroulante des SUPPORTS de la boutique —
  // jamais tout le stock (Timo, 14/09/2026, trois messages : « les supports
  // doivent être sélectionnés dans le devis, il y a les M8 et les M10 » ;
  // « on ne peut pas choisir, il reste choisi par défaut » — la liste ne
  // s'affichait qu'à partir de deux supports ; « une liste déroulante dans
  // laquelle seuls les supports sont sélectionnables, et non tous les
  // articles »). La liste s'affiche TOUJOURS, même avec un seul support, et
  // dit combien la boutique en a. Le premier est proposé d'office ; le choix
  // suit le brouillon (supportId) et le devis repris (par le nom).
  const articlesSupportsStock = supportsDuStock(produitsBoutique);
  ...
  const argumentsEnvoi = () => ({
    ...
    construire: () => construireDevis({
      profile, boutique, typeDevis: undefined, complement: {},
      besoins: {
        wh_jour: whParJour,
        ...
        <div className="space-y-2">
          {appareils.map((a) => (
            <div key={a.id} className="grid grid-cols-2 sm:grid-cols-5 gap-2 items-end">
              <Field label="Appareil"><ChampSuggestions placeholder="Ex : tv, frigo, clim…" valeur={a.nom} suggestions={propositionsAppareils} onChange={(v) => majAppareil(a.id, "nom", v)} onChoisir={(s) => choisirAppareil(a.id, s)} /></Field>
