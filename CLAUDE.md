...
### 🧾 LE REÇU D'UN VERSEMENT ET D'UNE RÉSERVATION PARTENT AUSSI (25/09/2026, « Lance avec ces deux textes »)
- Timo : « les réservations et les règlements de dette auront aussi les
  messages ? si oui, sous quel format ». Deux modèles UTILITY, sept trous
  chacun, textes proposés et acceptés tels quels (`TEXTE_RECU_REGLEMENT`,
  `TEXTE_RECU_RESERVATION`, lib/whatsappModeles.js) :
  **`recu_reglement`** — nom, montant VERSÉ, date, moyen (`moyenVersement` :
  « en espèces », « par Mobile Money (Flooz) »), n° de la dette, où elle en
  est (`situationDette` : « il reste X sur un total de Y », au dernier
  versement « votre compte est soldé, merci »), téléphone de la boutique ;
  **`recu_reservation`** — nom, date, boutique, n°, TOTAL, « avance X, reste
  Y » / « aucune avance, reste Y » / « entièrement payée », téléphone.
- **Les mêmes règles que le reçu de vente** : UNE fonction
  (`envoyerRecuSansQuestion`, src/whatsapp.js), sans question, **sans repli**
  (WhatsApp ne s'ouvre jamais), rien sans numéro, rien en formation ; la ligne
  entre dans 📲 WhatsApp sans donner la conversation ; ce qui s'est passé se
  lit discrètement sous le titre (`data-recu-whatsapp`), jamais une fenêtre.
  ⚠ Le mur = l'espace de la BOUTIQUE de la dette (ou de la caisse qui
  encaisse), jamais celui de qui clique.
- **Quatre gestes** : un versement dans 📋 Dettes (`encaisser`, sur la dette
  APRÈS le versement), un versement dans 🏠 Clients installés
  (`encaisserPose`), une réservation créée dans 📋 Dettes et une vente qui
  devient réservation dans 💰 Ventes. ⚠ **Une réservation n'envoie QUE son
  reçu de réservation** : son avance est dedans, on ne l'annonce pas deux fois.
  ⚠ Une dette CRÉÉE à la main dans 📋 Dettes (avec acompte) n'envoie rien :
  pas demandé.
- **Le n° de la dette ({{5}} / {{4}}) est celui du REÇU IMPRIMÉ** (25/09/2026,
  « lance le numéro calculé pour les vieilles dettes ») : les écrans passent
  `numeroDe: numeroRecuDette` (core.js — whatsappModeles.js n'importe rien) ;
  une vieille dette sans numéro enregistré reçoit « BMI-DET-2025-AB12 » au lieu
  de « — ». Un numéro enregistré (« BMID-DET-2026-0003 ») part tel quel : aucun
  préfixe n'est ajouté — l'exemple tapé chez YCloud ne sert qu'à Meta.
- Banc ㉘ (532), éprouvé en remettant quatre fautes (le repli remis, le mur
  retiré, un reçu de versement en plus sur la réservation, « il reste 0 F ») :
...
- **`recu_vente_detail`** (UTILITY, HUIT trous : client, date, boutique, reçu,
  **articles**, montant, paiement, téléphone — `TEXTE_RECU_VENTE_DETAIL`,
  ...
  articles ENTIERS, `LONGUEUR_MAX_ARTICLES` = ~~250~~ **500** (25/09/2026,
  « oui 500 » : 10 à 15 articles au lieu de 5 à 8) puis « + N autres
  articles » ; la liste ne prend que la place qui RESTE sous les 1 024
  caractères de Meta (`LIMITE_MESSAGE_META`), jamais plus de 500. Les lignes viennent de l'écran (`lignesVente`, ce fichier
  n'importe rien).
...
- Timo : « le bon de retour et de reprise n'envoie pas le message WhatsApp
  automatiquement par le numéro de BMI » — pas une panne : construits le
  14/09 pour ouvrir WhatsApp sur l'appareil, jamais repris ensuite. Avec
  l'exemple du bon d'EZO ENERGY, il a voulu **la forme du bon imprimé**.
- **`bon_reprise`** (UTILITY, 12 trous : boutique, adresse, téléphone, n°,
  date, reçu d'origine, client, article, motif, valeur, réglé, établi par) et
  **`bon_retour`** (UTILITY, 11 trous : …, article, motif, frais, établi par)
  — `TEXTE_BON_REPRISE` / `TEXTE_BON_RETOUR`, `envoiBon`, `reglementReprise`,
  `fraisRetour` (lib/whatsappModeles.js). Plusieurs lignes dans le modèle,
  jamais dans un trou. ⚠ **La dernière ligne est « Merci de votre confiance.
  BMI TOGO », JAMAIS le mot de la boutique** (« Les articles vendus ne sont
  ni repris, ni échangés » contredirait un bon de reprise). Pas de ligne de
  formation (rien ne part en formation). Adresse vide → « Lomé, Togo »,
  téléphone vide → numéro BMI, client vide → « Non renseigné ».
- ⚠ **UN bon = UN article**, quelle que soit la quantité (une reprise et un
  échange ne prennent qu'un article) : trois articles rendus = trois bons,
  trois messages. Grouper plusieurs articles dans une reprise n'a pas été
  demandé (proposé seulement si ça arrive souvent).
- **Juste après la reprise ou l'échange** : le bon part TOUT SEUL, sans
  question et sans repli (la règle des reçus), la note sous le titre le dit
  (`data-recu-whatsapp`) ; déjà parti, « Envoyer par WhatsApp » ne s'offre
  plus, il reste Imprimer / Plus tard. **Depuis le bouton 🧾 de la ligne** :
  une QUESTION, puis le numéro BMI ; échec → le motif en français, puis le
  bon complet sur l'appareil ; sans numéro ou en formation → l'appareil,
  comme avant. UNE fonction (`envoyerBonDuNumeroBmi`, Ventes.jsx) ; le mur =
  la BOUTIQUE de la vente ; la ligne du fil ne donne la conversation à
  personne.
- **À faire par Timo : créer les deux modèles chez YCloud** (utility, fr),
  avec les textes du message du 25/09. Si Meta refuse pour « trop de trous »,
  on retire l'adresse et on recrée sous un AUTRE nom. ~4 F le bon.
- Banc ㉙ (`verifier-whatsapp`, 564), éprouvé en remettant cinq fautes (le
  mur ouvert, l'envoi automatique retiré, la question retirée, la limite
...
### 📄🔑 LE PREMIER DEVIS D'UN CLIENT : SES ACCÈS PUIS LE DEVIS, EN DEUX MESSAGES DU NUMÉRO BMI (25/09/2026)
- Timo : « LES DEVIS NE PARTENT TOUJOURS PAS AVEC LE NUMÉRO DE BMI ». La
  cause était NOTRE règle du 19/09 : le **premier** devis d'un client jamais
  contacté partait à la main, parce qu'il porte les codes.
- ⚠⚠ **UN SEUL MODÈLE « DEVIS + ACCÈS » EST IMPOSSIBLE CHEZ META** : essayé
  sous trois noms le 25/09/2026 (`devis_premier` en marketing, `devis_acces`
  en utility, `acces2`) → **refusé trois fois** (INCORRECT_CATEGORY). Meta ne
  mélange pas une OFFRE et des ACCÈS. Timo a supprimé `devis_premier` ; il a
  été RETIRÉ du code (une règle qui ne commande plus rien ne reste pas).
  **Ne pas le reproposer, sous aucun nom.**
- **La règle, telle que Timo l'a reformulée** : « un nouveau client… les
  accès s'envoient automatiquement avec le modèle espace et, à une seconde
  phase, le devis avec devis disponible… l'utilisateur voit juste deux
  messages envoyés » → « lance ».
  - **Ses accès déjà partis** (`accesDejaEnvoyes`, lib/whatsappModeles.js :
    `clientDejaContacte` OU une ligne `espace` dans le fil qui porte
    `wa_acces.client_id` — création du compte, « ↻ Renvoyer ») → le devis
    part **SEUL** par `devis_disponible` (~14 F). On ne paie pas deux fois.
  - **Jamais partis** (client tapé dans le devis) → `espace` **D'ABORD**
    (`sansRepli`, sinon WhatsApp s'ouvrirait deux fois), puis
    `devis_disponible` (~28 F en tout). La ligne d'accès s'écrit dans
    📲 WhatsApp **masquée** (`messagesAvecLigneAcces`, règle du 23/09).
  - ⚠ **Si `espace` ne part pas** (formation, réseau, refus, mot de passe
    qu'on ne sait pas recalculer) → le devis ne part **PAS** non plus du
    numéro BMI : WhatsApp s'ouvre avec le texte d'avant, codes compris
    (`premierContact`), et le motif se dit en français sauf s'il est attendu.
    Sinon le client recevrait un lien vers un espace où il ne saurait pas
    entrer.
  - Le mur ne change pas : l'espace du DEVIS, calculé UNE fois pour les deux
    envois.
- `espace` est de nouveau le **SEUL** modèle qui porte un secret. Banc ㉚
  (`verifier-whatsapp`, 572), éprouvé en remettant quatre fautes (`espace`
  envoyé à un client qui a déjà ses accès, le devis parti du numéro BMI sans
...
- **UN SEUL BONJOUR, ET LA QUANTITÉ SE DEMANDE** (capture Timo, 25/09/2026 :
  ...
  le vendeur corrige) — ~~dit à Timo, pas changé sans lui~~ **RETOURNÉ le
  25/09/2026** (Timo : « une ampoule (c'est bon), 1 ampoule (c'est aussi bon),
  les ampoules (ce n'est pas bon), quelques lumières (pas bon aussi) ») :
  `lireAppareils` pose `qteDite` (vrai si un chiffre ou un nombre écrit —
  « un », « une », « dix » — ouvre la ligne), et **`estimationSolaire` REFUSE
  toute ligne à `qteDite === false`** (« Il manque le NOMBRE pour : … »,
  aucun montant permis). ⚠ Seul `false` refuse : une ligne saisie à la main
  n'a pas le drapeau. ⚠ **« 🔆 Préparer le devis » garde 1 d'office**, et
  c'est voulu : là, le vendeur relit et corrige. Éprouvé en retirant le
  refus, puis en marquant le nombre « dit » d'office : les contrôles tombent.
- **« QUE FAITES-VOUS ? » : LE TEXTE DE TIMO, MOT POUR MOT** (24/09/2026 au
  ...
  solaire, 🏠 Automatisation des bâtiments, 🛒 Vente d'équipements, 📍, 👤
  « conseiller » — est DANS la consigne avec l'ordre de le recopier TEL QUEL
  (seule réponse qui peut dépasser 6 lignes). Il passe le juge (aucun
  montant, aucun sujet réservé, 860 caractères). ⚠ C'est une CONSIGNE, pas
  une phrase posée par le serveur : l'IA le recopie, le banc ne peut pas
  garantir qu'elle ne changera jamais un mot. ⚠ Il ne cite pas la VMC (son
  texte) ; la consigne la garde dans les activités. ⚠ En mode 🔢 menu à
  chiffres, « que faites-vous » n'a pas ce texte (le menu montre ses lignes).
...
- **L'estimation part SANS guillemets** (Timo, 24/09/2026 au soir : « retire
  les guillemets » — au premier vrai échange l'IA l'avait écrite entre « … »,
  comme une citation). La consigne le demande, et `sansGuillemetsAutour`
  (lib/assistantIA.js) retire quand même les guillemets autour de la phrase
  de l'outil si l'IA en met — la phrase et ses chiffres ne bougent pas, les
  espaces ordinaires / insécables se valent, un autre « … » du texte reste.
...
- ⚠ **LE MUR** : l'estimation ne regarde que les boutiques RÉELLES
  ...
  contrôle tombe.
- **Le banc** (`verifier-whatsapp` ㉓, **470**) exerce la phrase de MANDA sur
...
- **Le NOM reste figé pendant le défilement horizontal** (Timo, le jour même :
  « figer le nom du client… comme dans stock ») : la colonne Nom passe en
  PREMIÈRE (avant la Date), par LA règle commune `enTeteFige` / `celluleFigee`,
  fond orange gardé sur une relance en retard, `min-w-[170px]` (sinon
  « Demande de l'assistant WhatsApp » s'écrasait sur quatre lignes au
  téléphone). Mesuré dans Chromium à 420 px : le nom reste à 17 px quand la
  date part à −413 px.
- Banc ㉗ (504), éprouvé en remettant le besoin dans Localisation et en
...
| **📘 Livret de formation** (décision « c », 22/09/2026 : « ma journée » par métier, PUIS les écrans un par un, PUIS les règles de la maison — Word + PDF, 24 pages). Les MOTS vivent dans `scripts/livret-contenu.mjs`, la mise en page dans `scripts/livret-formation.mjs` ; `npm run livret-formation` refait les deux fichiers dans `docs/`. ⚠ Le sommaire est ÉCRIT (deux passes : on lit sur quelle page tombe chaque titre dans le PDF) parce que LibreOffice ne calcule pas un champ Word à la conversion — il sortait vide. ⚠ Il faut `libreoffice-writer` ET `poppler-utils` (le conteneur n'avait que `libreoffice-core`). **Quand un écran change, on refait le livret** — un livret qui décrit un bouton disparu est pire qu'un livret absent. ⚠ **Le premier jet décrivait un bouton qui n'existe PAS** (« ✅ Valider la réception » sur un ravitaillement, à trois endroits) : pour un ravitaillement venu du magasin, **c'est le magasinier qui valide le bon (« ✅ Valider le ravitaillement ») et le stock bouge à cet instant, la boutique ne valide rien** ; « Valider la réception » n'existe que pour un TRANSFERT de stock entre boutiques. Trouvé en écrivant « le ravitaillement du gérant » (Timo, 22/09/2026 : « il manque le ravitaillement du gérant ») — **on relit le CODE avant d'écrire un bouton, jamais de mémoire** | Remis le 22/09/2026, ravitaillement du gérant ajouté le jour même | `docs/livret-formation.docx` / `.pdf` |
| **📖 MANUEL DE FORMATION OPÉRATIONNEL, par chapitre** (22/09/2026, « Lance le chapitre 1 » — après que Timo a montré le plan de ChatGPT : 25 chapitres par THÈME, **12 rubriques** par chapitre — objectif, qui, accès, procédure, boutons, ce qui se passe derrière, interdépendances, contrôles, erreurs, cas pratiques, exercice, critères — puis un examen pratique par poste). Les MOTS d'un chapitre vivent dans `scripts/manuel/chapitre-NN.mjs`, la mise en page dans `scripts/manuel-formation.mjs` ; `npm run manuel-formation` refait **un Word + un PDF par chapitre** ET le manuel complet, dans `docs/manuel/`. Chaque chapitre se termine par une **fiche de validation** à signer (Acquis / À revoir). ⚠ **Chaque bouton, chaque chiffre vient du CODE** (la leçon du livret). ⚠ Sa liste est corrigée là où elle ne colle pas à l'application : pas de module « achats » (ch. 10 = fournisseurs et entrées), pas de « garage » (ch. 12 = portail et devis sans calcul). ⚠ **« Bien modéliser le document, pour qu'il soit joli »** (Timo, le jour même) : couverture bleue, rubriques numérotées sur bandeau, encadrés À SAVOIR / ATTENTION / RÈGLE DE LA MAISON, étapes numérotées, cas pratiques en cartes — on rend les pages en images et on les REGARDE avant d'envoyer. **Le livret « ma journée par métier » sera ABSORBÉ** (il devient l'examen pratique par poste, à la fin) : deux documents sur les mêmes boutons finiraient par se contredire. Un chapitre à la fois, remis à Timo, validé, puis le suivant. ⚠ **Écrire un chapitre, c'est aussi relire les PHRASES de l'écran** : le chapitre 3 (Clients, 22/09/2026) a trouvé DEUX phrases qui mentaient — 👥 Utilisateurs annonçait un mot de passe client « 4 derniers chiffres + 2 premières lettres » (l'algorithme d'AVANT la 2.98.68 : c'est un mélange de 6 caractères, `motDePasseClient`), et le chapitre 2 l'avait RECOPIÉE ; 🙋 Créer un client écrivait « WhatsApp s'est ouvert » en dur, faux depuis que les accès partent du numéro BMI. Les deux corrigées le jour même (2.101.305), le chapitre 2 aussi. ⚠⚠ **LA NUMÉROTATION EST CELLE DU PLAN DE ChatGPT, retenu par Timo** — 1 Connexion · 2 Utilisateurs · 3 Clients · **4 Prospects** · **5 Ventes et commandes** · 6 Caisse · 7 Dettes · 8 Stocks · 9 Ravitaillement et transferts · 10 Fournisseurs · 11 Dimensionnement solaire · 12 Portail et devis sans calcul · 13 Devis et proformas · 14 Contrats · 15 Chantiers / travaux · 16 Commissions et primes · 17 Dépenses · 18 Salaires · 19 Outillage · 20 Messagerie et WhatsApp · 21 Espace client · 22 Rentabilité / historique · 23 Paramètres · 24 Hors connexion · 25 Incidents. Les renvois « chapitre N » des chapitres écrits suivent CETTE liste. Le 22/09/2026 au soir j'ai annoncé par erreur « chapitre 4 (Ventes) » ; Timo a dit « Lance le chapitre 4 » : le chapitre Ventes et commandes a été écrit et **numéroté 5**, sa vraie place ; Timo : « J'ai dit de lancer le chapitre 4, tu me livres chapitre 5 » → le 4 (Prospects) a été écrit dans la foulée. ⚠ Il a trouvé UNE phrase d'écran fausse de plus : 🧲 Prospects disait « Demandez à l'administrateur d'en créer dans Paramètres » pour les catégories, alors qu'elles se gèrent DANS 🧲 Prospects (cadre « Catégories de prospects ») — corrigée (2.101.306). **On livre le numéro demandé, pas le suivant.** | Chapitres 1 à 6 remis le 22/09/2026 (le 6, Caisse et encaissements, écrit d'après Caisse.jsx, lib/cloture.js, lib/versements.js, lib/validationDepenses.js, lib/caissesMobiles.js — aucune phrase d'écran fausse trouvée) ; chapitre 7 (Dettes et paiements) remis le 23/09/2026, d'après Dettes.jsx, lib/reglement.js, lib/rappels.js, lib/whatsappModeles.js, impression.js — aucune phrase fausse non plus ; chapitre 8 (Stocks) remis le 23/09/2026, d'après Stocks.jsx, lib/transfertsStock.js, lib/importStock.js, lib/pompes.js, impression.js (étiquette), notifications.js — aucune phrase fausse ; chapitre 9 (Ravitaillement et transferts) remis le 23/09/2026, d'après Ravitaillement.jsx, la partie magasin de Stocks.jsx, le choix « Stock insuffisant » de Ventes.jsx, lib/transfertsStock.js, notifications.js, impression.js (le bon) — aucune phrase fausse ; chapitre 10 (Fournisseurs et entrées de stock) remis le 23/09/2026, d'après Fournisseurs.jsx, la part fournisseur et les entrées de Stocks.jsx, lib/importStock.js, lib/calculs.js — UNE phrase fausse trouvée et corrigée (2.101.311) : le refus d'un règlement trop élevé renvoyait à un bouton « + Dette » qui s'appelle « + Commande » ; ⚠ à dire à Timo : un règlement de fournisseur crée sa dépense « Achat marchandises » SANS passer par la validation du DG (le geste est réservé au gérant et à l'administrateur, c'est là qu'est le contrôle) — le chapitre le dit tel quel ; ⚠ dans les TABLEAUX du manuel, « ⇄ » et « ⏳ » sortent en carré vide à la conversion PDF (le texte courant les rend bien) : on écrit le mot dans une cellule de tableau ; chapitre 11 (Dimensionnement solaire) remis le 25/09/2026, d'après Solaire.jsx, Partages.jsx, devisCommun.js, Brouillons.jsx, lib/solaire.js, lib/choixSolaire.js, Parametres.jsx — aucune phrase fausse ; son cas pratique chiffré (10 ampoules 15 W · 12 h, TV 100 W · 6 h, congélateur 200 W · 24 h → 7 200 Wh/j, 1 800 Wc, 157 Ah, 0,90 kW, 44 A ; 4 panneaux → 9 m → 3 barres, 9 supports, 16 étriers) a été CALCULÉ avec lib/solaire.js, pas de tête ; ⚠ la date du manuel (`DATE_MANUEL`) est commune à tous les chapitres : elle est passée au 25/09/2026 ; chapitres 12 à 25 à écrire | `docs/manuel/` |
